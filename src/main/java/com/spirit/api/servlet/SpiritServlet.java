...
public class SpiritServlet extends HttpServlet implements Filter {

    private static final long serialVersionUID = 5686655395749077671L;

    private static final Logger LOGGER = Logger.getLogger(SpiritServlet.class.getName());
    ...
    private List<SpiritServletFilter> servletFilters;
    ...
    private class InternalWebComponent extends WebComponent {
        ...
        protected WebApplication create() {
            return SpiritServlet.this.create();
        }
        ...
        protected void configure(WebConfig wc, ResourceConfig rc, WebApplication wa) {
            ...
            SpiritServlet.this.configure(wc, rc, wa);
        }

        @Override
        protected void initiate(ResourceConfig rc, WebApplication wa) {
            SpiritServlet.this.initiate(rc, wa);
        }

        @Override
        protected ResourceConfig getDefaultResourceConfig(Map<String, Object> props,
                                                          WebConfig wc) throws ServletException  {
            return SpiritServlet.this.getDefaultResourceConfig(props, wc);
        }
    }
    ...
    public SpiritServlet() {
        this.app = null;
    }

    public SpiritServlet(ResourceConfig rc){
        this.app = null;
        this.rc = rc;
    }

    public SpiritServlet(Class<? extends Application> appClass) {
        this.app = new DeferredResourceConfig(appClass);
    }

    public SpiritServlet(Application app) {
        this.app = app;
    }
...
//        }
    ...
    protected void initiate(ResourceConfig rc, WebApplication wa) {
        try{
            ApiLogger.info("SpiritServerlet rc is null: " + (this.rc == null));
            if(this.rc == null){
                wa.initiate(rc,
                        new SpringComponentProviderFactory(rc, getContext()));
            }else{
                wa.initiate(rc);
            }
...
//            wa.initiate(rc);
            ...
            ApiLogger.error("Exception occurred when initialization.", e);
            throw e;
        }
//        try {
...
//        }
    ...
    protected ConfigurableApplicationContext getContext() {
        ...
        if (contextConfigLocation == null) {
            ApiLogger.info("Using default applicationContext");
            return getDefaultContext();
        } else {
            ApiLogger.info("Creating new child context from " + contextConfigLocation);
...
//        return springContext;
    ...
    protected void afterInit(){
        servletFilters = new ArrayList<SpiritServletFilter>();
        servletFilters.add(new RequestThreadLocal());
    }
    ...
    public int service(URI baseUri, URI requestUri, final HttpServletRequest request, HttpServletResponse response)
        ...
        try{
            fireBeforeService(request);
            return webComponent.service(baseUri, requestUri, request, response);
        } catch (Exception e){
            ApiLogger.error("Servlet service error: ", e);
            return 500;
        } finally {
            fireAfterService(request);
        }

    }

    protected void fireBeforeService(HttpServletRequest request){
        for(SpiritServletFilter serviceListener:servletFilters){
            serviceListener.before(request);
        }

    }
    protected void fireAfterService(HttpServletRequest request){
        for(SpiritServletFilter serviceListener:servletFilters){
            try{
                serviceListener.after(request);
            }catch(Exception e){
                ApiLogger.error(String.format("fireAfterService error listener type:%s",
                        serviceListener.getClass().getSimpleName()), e);
            }
        }
    }
    ...
    public void init() throws ServletException {
        init(new WebConfig() {
            ...
            public String getName() {
                return SpiritServlet.this.getServletName();
            }

            public String getInitParameter(String name) {
                return SpiritServlet.this.getInitParameter(name);
            }

            public Enumeration getInitParameterNames() {
                return SpiritServlet.this.getInitParameterNames();
            }

            public ServletContext getServletContext() {
                return SpiritServlet.this.getServletContext();
            }

            public ResourceConfig getDefaultResourceConfig(Map<String, Object> props) throws ServletException {
                return SpiritServlet.this.getDefaultResourceConfig(props, SpiritServlet.this.getServletConfig());
            }
        });
    }
...
//            }
        ...
        queryParameters = new String(queryParameters.getBytes("ISO-8859-1"),"UTF-8");
    ...
    public void init(FilterConfig filterConfig) throws ServletException {
        ...
        init(new WebConfig() {
            ...
            public String getName() {
                return SpiritServlet.this.filterConfig.getFilterName();
            }

            public String getInitParameter(String name) {
                return SpiritServlet.this.filterConfig.getInitParameter(name);
            }

            public Enumeration getInitParameterNames() {
                return SpiritServlet.this.filterConfig.getInitParameterNames();
            }

            public ServletContext getServletContext() {
                return SpiritServlet.this.filterConfig.getServletContext();
            }

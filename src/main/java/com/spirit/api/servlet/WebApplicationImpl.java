...
import com.spirit.api.filter.MediaTypeRequestFilter;
...
import com.sun.jersey.server.impl.application.*;
...
import com.sun.jersey.server.impl.model.parameter.*;
...
public final class WebApplicationImpl implements WebApplication {
    ...
    private void _initiate(final ResourceConfig rc, final IoCComponentProviderFactory _provider) {
        ...
        // add MediaTypeRequestFilter
        // 暂时没有找到合适的地方来增加这个filter, 先直接加吧, 这个filter用于去掉.json后缀
        filterFactory.getRequestFilters().add(new MediaTypeRequestFilter());
    ...
    private void _handleRequest(final WebApplicationContext localContext,
        ...
        if (!rootsRule.accept(path, null, localContext)) {
            ApiLogger.info("WebApplicationImpl _handleRequest NotFoundException, path: " + path);
